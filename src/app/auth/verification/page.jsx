...
const Page = () => {
  ...
  const { sendOTP, verifyOTP, signUp } = ApiFile;
  const { postData, header3, signUpData } = useApiClient();
  ...
  const HandleSignUp = debounce(() => {
    ...
    const apiData = {
      email: dataDey?.email,
      fname: dataDey?.firstName,
      lname: dataDey?.lastName,
      phone: dataDey?.phoneNumber,
      password: dataDey?.password,
      code: otp,
    };
    postData(apiSignup, apiData, header3)
      .then((res) => {
        setOtpLoading(false);
        if (res?.success) {
